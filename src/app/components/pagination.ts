...
type PaginationElementsNames =
  ...
  | "inactiveBullet"
  | "activeBullet";
type PaginationElements = Record<
  PaginationElementsNames,
...
class Pagination extends HTMLElement {
  ...
  #classes: PaginationClasses = {
    ...
    prevBtn: [
      "prev-page-btn",
      "disabled:text-primary-black-45",
  ...
  renderBaseTemplate() {
    ...
    this.#elements.bulletsWrapper.classList.add(
      ...this.#classes["bulletsWrapper"]
    );

    this.#elements.wrapper.append(
      this.#elements.prevBtn,
      this.#elements.bulletsWrapper,
  ...
  renderBullets() {
    ...
    if (this.showAll === true) {
      ...
      const min = Math.max(
        1,
        this.currentPage -
          (Number(this.currentPage === this.totalPages) + this.showedPages)
      );
      const max = Math.min(
        this.totalPages,
        this.currentPage +
          this.showedPages +
          Math.max(0, min - this.currentPage + this.showedPages)
      );
